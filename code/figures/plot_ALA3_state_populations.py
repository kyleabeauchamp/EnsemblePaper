...
for k, ff in enumerate(ALA3.ff_list):
    directory = "/home/kyleb/dat/lvbp/%s/raw/" % ff
...
for k,ff in enumerate(ALA3.ff_list):
    regularization = ALA3.regularization_strength_dict[ff]
...
for state,state_name in enumerate(state_name_list):
    ...
    plt.savefig(ALA3.outdir+"/state_%d_by_forcefield.pdf"%(state),bbox_inches='tight')
