...
for i, prior in enumerate(ALA3.prior_list):
    for j, ff in enumerate(ALA3.ff_list):
        ...
        #rms_lvbp[i,j] = np.mean(((predictions.T.dot(p) - measurements) / uncertainties) ** 2.0) ** 0.5  # Don't use average populations, use MCMC
        #rms_lvbp[i,j] = np.mean(((mu_lvbp - measurements) / uncertainties) ** 2.0).mean() ** 0.5
        #print(rms_lvbp[i,j])
